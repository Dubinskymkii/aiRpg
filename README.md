...
## Варианты начала новой кампании

Самый короткий вариант:

```text
*new new_campaign | Новая кампания
```

Если название уже придумано:

```text
*new total_war | Тотальная война
```

Если хочется сразу задать направление кампании, после `*new` можно обычным внеигровым сообщением уточнить жанр и фокус:

```text
*кампания про тяжёлую пехотную войну, почти без магии, основной фокус на боях, маршах и выживании
```

Или:

```text
*кампания про болотную окраину, охоту, странных животных и медленное раскрытие опасного мира
```

Или:

```text
*кампания про городскую авантюру: долги, банды, рынки, подземелья и немного магии
```

После этого агент должен использовать `core/world/BOOTSTRAP.md`, создать только минимальный мир для старта и попросить описание первого игрового персонажа.

Можно также сразу описать персонажа после `*new`, не дожидаясь отдельного вопроса:

```text
*new border_war | Пограничная война

Я обычный арбалетчик гарнизона. Опыт небольшой, хорошо вижу вдаль,
плохо переношу долгие марши и не люблю начальство.
```

Агент должен распознать это как создание кампании + создание первого игрового персонажа и не заставлять повторять уже данное описание.
